...
For readability, the heat maps shown in Figs.~\ref{fig:d1-xy-energy-A}--\ref{fig:d2-xz-energy-B} use a display-only manuscript rendering preset that applies mild zero-centered whitening, local smoothing, and isolated-pixel suppression. This post-processing is used only for visualization, to mimic the cleaner Mathematica-style presentation of the earlier draft. All reported success fractions, principal-stress margins, and optimization diagnostics are computed from the unsmoothed fields exported by the pipeline and are not recalculated from the display-processed images.
